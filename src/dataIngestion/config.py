...
class Config:
    ...
    def from_file(cls, config_path: str) -> "Config":
        ...
        print(
            "Warning: JSON config files are deprecated. Please use environment variables or .env files."
        )
    ...
    def validate(self) -> bool:
        ...
        if not self.mongodb_database:
            raise ValueError(
                "MongoDB database name is required. Set MONGODB_DATABASE environment variable."
            )

        if not self.mongodb_collection:
            raise ValueError(
                "MongoDB collection name is required. Set MONGODB_COLLECTION environment variable."
            )
