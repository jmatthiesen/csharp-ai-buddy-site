...
class DocumentPipeline:
    ...
    def convert_to_markdown(self, document: Document) -> str:
        ...
        try:
            ...
            logger.warning(
                f"Error converting to markdown, falling back to document ID conversion: {e}"
            )
            try:
    ...
    def categorize_document(
        self, markdown_content: str, existing_tags: Optional[List[str]] = None
    ) -> List[str]:
        """
        ...
        try:
            ...
            for i, chunk_content in enumerate(chunks):
                ...
                chunk_document = Document(
                    documentId=(
                        f"{document.documentId}#chunk_{i}"
                        if len(chunks) > 1
            ...
            logger.info(
                f"Successfully processed document into {len(processed_documents)} chunks: {document.documentId}"
            )
        ...
        try:
            ...
            for chunk in document_chunks:
                try:
                    ...
                    logger.warning(
                        f"Document chunk with ID {chunk.documentId} already exists, skipping"
                    )
                    continue
    ...
    def update_document(self, document: Document) -> bool:
        ...
        try:
            ...
            result = self.documents_collection.update_one(
                {"documentId": document.documentId}, {"$set": document.to_dict()}
            )
    ...
    def list_documents(self, tags: Optional[List[str]] = None, limit: int = 50) -> List[Document]:
        ...
        try:
            ...
            documents_data = list(
                self.documents_collection.find(filter_query).sort("createdDate", -1).limit(limit)
            )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get tag statistics
            tag_stats = list(
                self.documents_collection.aggregate(
                    [
                        {"$unwind": "$tags"},
