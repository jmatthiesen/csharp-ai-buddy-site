...
class RSSFeedMonitor:
    ...
    def _is_item_processed(self, feed_url: str, item_id: str) -> bool:
        ...
        try:
            doc = self.processed_items_collection.find_one(
                {"feed_url": feed_url, "item_id": item_id}
            )
    ...
    def _process_feed_item(
        self, feed_item: FeedParserDict, subscription: RSSFeedSubscription
    ) -> bool:
        """
        ...
        try:
            ...
            if hasattr(feed_item, "tags"):
                categories = [
                    tag.term for tag in feed_item.tags if hasattr(tag, "term")
                ]
            ...
            stored_ids = processing_context.processing_metadata.get(
                "stored_chunk_ids", []
            )
            ...
            logger.info(
                f"Processed RSS item into {len(stored_ids)} chunks: {raw_document.title}"
            )
            return True

        except Exception as e:
            logger.error(
                f"Error processing RSS item {feed_item.get('title', 'Unknown')}: {e}"
            )
            return False
    ...
    def check_feed(self, subscription: RSSFeedSubscription) -> int:
        ...
        try:
            logger.info(
                f"Checking RSS feed: {subscription.name} ({subscription.feed_url})"
            )
            ...
            for feed_item in feed.entries:
                ...
                if self._process_feed_item(feed_item, subscription):
                    ...
                    published_date = None
                    if (
                        hasattr(feed_item, "published_parsed")
                        and feed_item.published_parsed
                    ):
                        published_date = datetime(*feed_item.published_parsed[:6])

                    if published_date and (
                        latest_item_date is None or published_date > latest_item_date
                    ):
            ...
            self.subscriptions_collection.update_one(
                {"feed_url": subscription.feed_url},
                {
                    "$set": {
                        ...
                        "last_item_date": (
                            latest_item_date.isoformat() if latest_item_date else None
                        ),
                        "updated_date": datetime.now(timezone.utc),
                    }
                },
            ...
            logger.info(
                f"Processed {processed_count} new items from {subscription.name}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="RSS Feed Monitor for RAG Data Pipeline"
    )
    subparsers = parser.add_subparsers(dest="command", help="Available commands")

    # Add subscription command
    add_parser = subparsers.add_parser(
        "add-subscription", help="Add a new RSS feed subscription"
    )
    add_parser.add_argument("--feed-url", required=True, help="RSS feed URL")
    add_parser.add_argument(
        "--name", required=True, help="Human-readable name for the feed"
    )
    ...
    remove_parser = subparsers.add_parser(
        "remove-subscription", help="Remove an RSS feed subscription"
    )
    remove_parser.add_argument(
        "--feed-url", required=True, help="RSS feed URL to remove"
    )

    # List subscriptions command
    list_parser = subparsers.add_parser(
        "list-subscriptions", help="List all RSS feed subscriptions"
    )

    # Check feeds command
    check_parser = subparsers.add_parser(
        "check-feeds", help="Check all enabled RSS feeds"
    )
    ...
    cleanup_parser = subparsers.add_parser(
        "cleanup", help="Clean up old processed items"
    )
    cleanup_parser.add_argument(
        "--days", type=int, default=30, help="Days of processed items to keep"
    )

    # Launch Streamlit UI command
    ui_parser = subparsers.add_parser(
        "launch-ui", help="Launch the Streamlit UI for RSS Feed Monitor"
    )
    ...
    try:
        if args.command == "add-subscription":
            ...
            for sub in subscriptions:
                print(
                    f"{sub.name}: {sub.feed_url} ({'enabled' if sub.enabled else 'disabled'})"
                )
