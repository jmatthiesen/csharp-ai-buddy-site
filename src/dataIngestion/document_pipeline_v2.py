...
from host_handlers import HostHandler, GitHubHostHandler, FallbackHostHandler
...
class DocumentPipeline:
    ...
    def __init__(self, config: Config, what_if_mode: bool = False):
        ...
        if what_if_mode:
            ...
            self.documents_collection = self.db[
                config.mongodb_collection
            ]  # For document summaries
            self.chunks_collection = self.db[
    ...
    def process_document(self, raw_doc: RawDocument, **options):
        ...
        try:
            ...
            logger.info(
                f"Successfully processed and stored {len(chunk_ids)} chunks: {raw_doc.source_url}"
            )
            return context
    ...
    def _generate_summary(self, content: str, max_length: int = 140) -> str:
        ...
        try:
            ...
            if self.what_if_mode:
                print(
                    f"📋 WHAT-IF: Would call OpenAI to generate summary (max {max_length} chars)"
                )
                return (
            ...
            logger.warning(
                f"Error generating AI summary, falling back to truncation: {e}"
            )
    ...
    def _stage_summary_creation(self, context: ProcessingContext) -> None:
        ...
        try:
            context.raw_document.summary = self._generate_summary(
                context.markdown_content, self.default_summary_size
            )
    ...
    def _stage_markdown_conversion(self, context: ProcessingContext) -> None:
        ...
        try:
            ...
            if context.raw_document.content_type == "markdown":
                ...
                html_content_bytes = BytesIO(
                    context.raw_document.content.encode("utf-8")
                )
                result = self.markitdown.convert(
                    source=html_content_bytes, stream_info=StreamInfo(extension=".html")
                )
                context.markdown_content = result.markdown

            context.processing_metadata["markdown_length"] = (
                len(context.markdown_content) if context.markdown_content else 0
            )
    ...
    def _stage_link_extraction(self, context: ProcessingContext) -> None:
        ...
        try:
            ...
            extracted_links = self.extract_links_from_markdown(
                context.markdown_content, context.raw_document.source_url
            )

            # Apply host-specific processing to extracted links
            processed_links = self._apply_host_handlers(
                extracted_links, context.raw_document.source_url
            )

            # Extract host-specific metadata
            host_metadata = self._extract_host_metadata(
                context.raw_document.source_url, context.markdown_content
            )
            if host_metadata:
            ...
            if processed_links:
                logger.info(
                    f"Extracted {len(processed_links)} crawlable links from {context.raw_document.source_url}"
                )
            else:
                logger.debug(
                    f"No crawlable links found in {context.raw_document.source_url}"
                )
    ...
    def _stage_chunking(self, context: ProcessingContext) -> None:
        ...
        try:
            ...
            context.processing_metadata["total_content_length"] = sum(
                len(chunk) for chunk in context.chunks
            )
    ...
    def _stage_embedding_generation(self, context: ProcessingContext) -> None:
        ...
        try:
            ...
            if self.what_if_mode:
                print(
                    f"📋 WHAT-IF: Would generate embeddings for {len(context.chunks)} chunks using {self.config.embedding_model}"
                )
                # Create empty embeddings for what-if mode
                for chunk_content in context.chunks:
                    context.chunk_embeddings.append([])
                context.processing_metadata["embeddings_generated"] = len(
                    context.chunks
                )
            ...
            for i, chunk_content in enumerate(context.chunks):
                if (
                    chunk_content.strip()
                ):  # Only generate embeddings for non-empty chunks
                    try:
                        response = self.client.embeddings.create(
                            model=self.config.embedding_model, input=chunk_content
                        )
                    ...
                    context.chunk_embeddings.append(
                        []
                    )  # Empty embeddings for empty chunks

            context.processing_metadata["embeddings_generated"] = len(
                [e for e in context.chunk_embeddings if e]
            )
            context.mark_stage_complete("embedding_generation")
    ...
    def _stage_ai_categorization(self, context: ProcessingContext) -> None:
        ...
        try:
            if self.what_if_mode:
                print(
                    f"📋 WHAT-IF: Would call OpenAI for AI categorization of document content"
                )
            ...
            if context.chunks and context.chunks[0].strip():
                try:
                    ...
                    context.add_warning(
                        "AI categorization module not available, skipping categorization"
                    )
                    logger.warning(
                        "AI categorization module not available, skipping categorization"
                    )
    ...
    def _stage_finalization_and_storage(self, context: ProcessingContext) -> List[str]:
        ...
        try:
            ...
            if self.what_if_mode:
                ...
                print(
                    f"📋 WHAT-IF: Would clean up existing chunks for document: {source_url}"
                )
                ...
                for i in range(total_chunks):
                    ...
                    print(
                        f"📋 WHAT-IF: Would store chunk {i+1}/{total_chunks}: {mock_chunk_id}"
                    )
            ...
            if result.deleted_count > 0:
                logger.info(
                    f"Cleaned up {result.deleted_count} existing chunks for document before processing"
                )

            # Check for length mismatch
            if len(context.chunks) != len(context.chunk_embeddings):
                logger.error(
                    f"LENGTH MISMATCH: chunks={len(context.chunks)}, embeddings={len(context.chunk_embeddings)}"
                )
                raise ValueError(
                    f"Chunk/embedding length mismatch: {len(context.chunks)} vs {len(context.chunk_embeddings)}"
                )

            # Create and store chunks
            for i, (chunk_content, embeddings) in enumerate(
                zip(context.chunks, context.chunk_embeddings)
            ):
            ...
            context.mark_stage_complete("finalization_and_storage")
            logger.info(
                f"Successfully stored document summary and {len(stored_chunk_ids)} chunks for: {source_url}"
            )
            return stored_chunk_ids
    ...
    def get_chunk(self, chunk_id: str) -> Optional[Chunk]:
        ...
        try:
            chunk_data = self.chunks_collection.find_one(
                {"chunk_id": chunk_id}, {"_id": 0}
            )  # Exclude MongoDB _id
            if chunk_data:
                # Convert ISO strings back to datetime objects
                if chunk_data.get("created_date") and isinstance(
                    chunk_data["created_date"], str
                ):
                    chunk_data["created_date"] = datetime.fromisoformat(
                        chunk_data["created_date"]
                    )
                if chunk_data.get("indexed_date") and isinstance(
                    chunk_data["indexed_date"], str
                ):
                    chunk_data["indexed_date"] = datetime.fromisoformat(
                        chunk_data["indexed_date"]
                    )
    ...
    def get_document_chunks(self, source_url: str) -> List[Chunk]:
        ...
        try:
            ...
            cursor = self.chunks_collection.find(
                {"source_url": source_url}, {"_id": 0}  # Exclude MongoDB _id
            ).sort("chunk_index", 1)
    ...
    def get_document(self, document_id: str) -> Optional[Document]:
        ...
        try:
            doc_data = self.documents_collection.find_one(
                {"documentId": document_id}, {"_id": 0}
            )  # Exclude MongoDB _id
            if doc_data:
    ...
    def search_documents(
        ...
        try:
            ...
            cursor = (
                self.documents_collection.find(filter_query)
                .sort("indexedDate", -1)
                .limit(limit)
            )
    ...
    def search_chunks(
        ...
        try:
            ...
            cursor = (
                self.chunks_collection.find(filter_query)
                .sort("indexed_date", -1)
                .limit(limit)
            )
    ...
    def _apply_host_handlers(
        self, links: List[Dict[str, str]], source_url: str
    ) -> List[Dict[str, str]]:
        """Apply host-specific processing to extracted links."""
        try:
            ...
            for handler in self.host_handlers:
                if handler.can_handle(source_url):
                    processed_links = handler.process_extracted_links(
                        processed_links, source_url
                    )
                    logger.debug(
                        f"Applied {handler.name} host handler to {len(links)} links"
                    )
        ...
        try:
            ...
            for handler in self.host_handlers:
                if handler.can_handle(url):
                    handler_metadata = handler.extract_host_metadata(
        ...
        try:
            ...
            for index in existing_indexes:
                ...
                if index_name in ["documentId_1", "chunk_id_1"] and index.get("unique"):
                    logger.info(
                        f"Dropping old index from documents collection: {index_name}"
                    )
                    try:
            ...
            for index in existing_chunk_indexes:
                ...
                if index_name in ["documentId_1", "chunk_id_1"] and index.get("unique"):
                    logger.info(
                        f"Dropping old index from chunks collection: {index_name}"
                    )
                    try:
                        self.chunks_collection.drop_index(index_name)
                    except Exception as e:
            ...
            old_chunks_in_docs = self.documents_collection.count_documents(
                {"chunk_id": {"$exists": True}}
            )

            # Count old documents in chunks collection that should be documents (no chunk_id field)
            old_docs_in_chunks = self.chunks_collection.count_documents(
                {"chunk_id": None}
            )

            logger.info(
                f"Found {old_chunks_in_docs} chunks incorrectly stored in documents collection"
            )
            logger.info(
                f"Found {old_docs_in_chunks} documents incorrectly stored in chunks collection"
            )
            ...
            if old_chunks_in_docs > 0:
                ...
                logger.info(
                    f"Deleted {result.deleted_count} chunks from documents collection"
                )
            ...
            if old_docs_in_chunks > 0:
                ...
                total_deleted += result.deleted_count
                logger.info(
                    f"Deleted {result.deleted_count} documents from chunks collection"
                )
    ...
    def extract_links_from_markdown(
        self, markdown_content: str, base_url: str
    ) -> List[Dict[str, str]]:
        """
        ...
        try:
            ...
            for match in re.finditer(link_pattern, markdown_content):
                ...
                if link_url.startswith("/"):
                    ...
                    absolute_url = (
                        f"{base_parsed.scheme}://{base_parsed.netloc}{link_url}"
                    )
                ...
                try:
                    ...
                    if (
                        ...
                        and url_path.startswith(base_path)
                        and (
                            absolute_url.endswith(".html")
                            or absolute_url.endswith(".md")
                            or absolute_url.endswith(".markdown")
                            or not parsed_url.path.split("/")[-1].count(".")
                        )
                    ...
                    logger.warning(
                        f"Could not parse URL {absolute_url}: {url_parse_error}"
                    )
                    continue
            ...
            logger.info(
                f"Extracted {len(unique_links)} crawlable links from {base_url}"
            )
            return unique_links
