        ...
        while i < len(lines):
            ...
            if stripped.startswith('```'):
                ...
                if current_size + code_size > chunk_size and current_chunk:
                    ...
                    current_size = len('\n'.join(section_headers)) + 1 + code_size if section_headers else code_size
                else:
            ...
            if stripped.startswith(('-', '*', '+')) or (stripped and stripped[0].isdigit() and '.' in stripped[:5]):
                ...
                while i < len(lines):
                    ...
                    if (list_stripped.startswith(('-', '*', '+')) or 
                        (list_stripped and list_stripped[0].isdigit() and '.' in list_stripped[:5]) or
                        (list_line.startswith('  ') or list_line.startswith('\t')) or  # Indented continuation
                        not list_stripped):  # Empty line within list
                ...
                if current_size + list_size > chunk_size and current_chunk:
                    ...
                    current_size = len('\n'.join(section_headers)) + 1 + list_size if section_headers else list_size
                else:
            ...
            while i < len(lines):
                ...
                if (para_stripped.startswith('#') or 
                    ...
                    (para_stripped and para_stripped[0].isdigit() and '.' in para_stripped[:5]) or
                    para_stripped.startswith('```') or
                    '|' in para_line):
                    break
            ...
            if paragraph_lines:
                if current_size + paragraph_size > chunk_size and current_chunk:
                    ...
                    current_size = len('\n'.join(section_headers)) + 1 + paragraph_size if section_headers else paragraph_size
                else:
