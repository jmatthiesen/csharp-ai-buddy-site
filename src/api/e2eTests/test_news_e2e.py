...
class TestNewsEndpointE2E:
    ...
    def test_news_sorted_newest_first_real_db(self, client, test_news_data):
        ...
        test_articles = [
            item for item in data["news"] if item.get("id", "").startswith("e2e-test-")
        ]

        # We should have all 3 test articles
        assert (
            len(test_articles) >= 3
        ), f"Expected at least 3 test articles, found {len(test_articles)}"
        ...
        newest_idx = next(
            i for i, a in enumerate(test_articles) if "Newest" in a["title"]
        )
        middle_idx = next(
            i for i, a in enumerate(test_articles) if "Middle" in a["title"]
        )
        oldest_idx = next(
            i for i, a in enumerate(test_articles) if "Oldest" in a["title"]
        )

        # Verify sorting: newest should come before middle, middle before oldest
        assert (
            newest_idx < middle_idx
        ), f"Newest article (index {newest_idx}) should come before middle article (index {middle_idx})"
        assert (
            middle_idx < oldest_idx
        ), f"Middle article (index {middle_idx}) should come before oldest article (index {oldest_idx})"

        print(
            f"✅ Sorting verified: Newest (idx {newest_idx}) < Middle (idx {middle_idx}) < Oldest (idx {oldest_idx})"
        )
